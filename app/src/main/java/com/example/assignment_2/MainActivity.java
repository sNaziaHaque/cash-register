...
public class MainActivity extends AppCompatActivity implements View.OnClickListener, AdapterView.OnItemClickListener {
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.buttonBuy) {
            if (!totalTV.getText().toString().equals("")) {
                showAlert(); // Show alert is the buy dialog
            } else {
        ...
        } else if (view.getId() == R.id.buttonManager) {
            // Button Manager
            Intent intentManager = new Intent(MainActivity.this, ManagerActivity.class);
            startActivity(intentManager);
        } else {
            // Default case for other buttons 1,2,3,4,5,6,7,8,9,0
            Button b = (Button) view;
            String num1 = quantityTV.getText().toString(); // By default, the quantity text is empty

            // Assign text from the button to num2
    ...
    private void showAlert() { //Buy confirmation Dialog
        ...
        builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            //setonClickListener to Ok Button
